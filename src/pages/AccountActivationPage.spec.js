...
describe("Account Activation Page", () => {
    ...
    beforeEach(() => {
        ...
        server.use(
            rest.post("/api/1.0/users/token/:token", (req, res, ctx) => {
                if(req.params.token === "5678") return res(ctx.status(400), ctx.json({message: "Activation failure"}));
                counter += 1;
    ...
    it("displays activation succes message when token is correct", async () => {
        setup("1234");
